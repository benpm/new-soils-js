...
async fn empty_username_is_rejected_and_server_survives() {
    ...
    bad.send(&ClientMsg::Login { name: String::new(), password: String::new(), signup: true, protocol: soils_protocol::PROTOCOL_VERSION })
        .await;
    let msg = bad
...
async fn duplicate_username_is_rejected() {
    ...
    b.send(&ClientMsg::Login { name: "dup".into(), password: "other".into(), signup: true, protocol: soils_protocol::PROTOCOL_VERSION })
        .await;
    let msg = b
        .recv_until(|m| match m {
            ServerMsg::LoginError { message } => Some(message),
...
async fn login_to_missing_account_is_rejected() {
    ...
    c.send(&ClientMsg::Login { name: "ghost".into(), password: "pw".into(), signup: false, protocol: soils_protocol::PROTOCOL_VERSION })
        .await;
    let msg = c
        .recv_until(|m| match m {
            ServerMsg::LoginError { message } => Some(message),
...
async fn pre_auth_messages_are_ignored_then_connection_works() {
    ...
    c.send(&ClientMsg::ViewRadius { radius: 3, full_streams: false }).await;
...
async fn oversized_view_radius_is_clamped_and_stable() {
    ...
    a.send(&ClientMsg::ViewRadius { radius: u8::MAX, full_streams: false }).await;
